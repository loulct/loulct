# About me

#### [Github Pages](https://loulct.github.io) / [LinkedIn](https://www.linkedin.com/in/lecart-louis)

## Employment

### <sub>2024 - ...</sub> Research Engineer at _EDF Labs Paris-Saclay_

- Full-Stack development (Python / TypeScript) of SCADA software (Supervisory Control and Data Acquisition) for embedded systems
- Behavior-Driven test automation (Robotframework / Selenium / Docker)
- PR/MR Review
- CI/CD

### <sub>2023 – 2024</sub> Research Engineer at _CEA-List Paris-Saclay_

- Kernel splitting for distributed systems (C++)
- HMI development (Java / XML)
- Plugin integration (Python)

### <sub>2023</sub> Data Analyst at _Spliit_

- Pipeline ETL development (Python / TypeScript / PostgreSQL)

### <sub>2021-2022</sub> Research Engineer at _Version Net_

- Full-Stack development (AngularJS / .NET / C# / SQL Server)

## Education

### <sub>2024 - 2027</sub> B.A. Philosophy, Paris-Nanterre University
### <sub>2018 - 2023</sub> M.Sc. Computer Science, Paris Higher Institute of Electronics
- Specialization: Information Systems Architecture
- University exchange: **Chalmers University of Technology**

## Tech Stack

![Languages](https://skillicons.dev/icons?i=python,java,cpp)

![Frontend Technologies](https://skillicons.dev/icons?i=ts,js)

![Database](https://skillicons.dev/icons?i=postgres,mysql,mongodb)

![Frontend Technologies](https://skillicons.dev/icons?i=next,vite)

![Package Managers](https://skillicons.dev/icons?i=npm,gradle,maven)

![Text Editors](https://skillicons.dev/icons?i=latex,neovim)

![Toolkit](https://skillicons.dev/icons?i=flask,qt,threejs)

![OS](https://skillicons.dev/icons?i=debian,arch)
